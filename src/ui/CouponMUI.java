...
public class CouponMUI extends JFrame implements ActionListener {

	private JPanel contentPane;
	private JTextField ShopName;
	private JButton button;
	private JButton button_1;
	List<BeanCoupon> allshop=null;
	private Object tblShopTitle[]=BeanCoupon.ShopTitle;
	...
	private BeanCoupon curShop=null;
	private JButton button_4;
	private JButton button_5;
	
	private void reloadShopTable() {
		try {
			allshop=CouponMannager.loadallShop();
		} catch (BaseException e) {
			JOptionPane.showMessageDialog(null, e.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
			return;
		}
		tblShopData =  new Object[allshop.size()][BeanCoupon.ShopTitle.length];
		for(int i=0;i<allshop.size();i++){
			for(int j=0;j<BeanCoupon.ShopTitle.length;j++)
	...
	public CouponMUI() {
		setTitle("\u4F18\u60E0\u5238\u7BA1\u7406");
		 this.dataTableShop.addMouseListener(new MouseAdapter (){
				...
				public void mouseClicked(MouseEvent e) {//点鼠标事件
					int i=CouponMUI.this.dataTableShop.getSelectedRow();
					if(i<0) {
						return;
					}
					CouponMUI.this.reloadShopDetails(i);
				}
		...
		JLabel label = new JLabel("\u4F18\u60E0\u5238\u540D");
		label.setFont(new Font("宋体", Font.PLAIN, 15));
		label.setBounds(135, 10, 86, 18);
	...
	public void actionPerformed(ActionEvent ac)
	{
		...
		{
			CouponModUI frame=new CouponModUI(curShop);
			frame.setVisible(true);
			//this.setVisible(false);
		}
